...
from envs.sim_env import UAVEnv
from ddpg.buffer import MultiAgentReplayBuffer
import time
import pandas as pd
import os
...
def save_image(env_render, filename):
    ...
    image = Image.fromarray(
        env_render, "RGBA"
    )  # Use 'RGBA' mode since the buffer includes transparency
...
if __name__ == "__main__":
    ...
    memory = MultiAgentReplayBuffer(
        1000000, critic_dims, actor_dims, n_actions, n_agents, batch_size=256
    )
    ...
    for i in range(N_GAMES):
        ...
        if i % PRINT_INTERVAL == 0 and i > 0:
            print(
                "episode",
                i,
                "average score {:.1f}".format(avg_score),
